...
public class DataConvertor {
	...
	public List<Marker> Gachon_load(String url){
		GachonDataProcessor gachonProcessor = new GachonDataProcessor();

		try {
			return gachonProcessor.load(url);
		} catch (JSONException e) {
			/* Find Other Away to notify Error, for now Hide this error
			 MixView.CONTEXT.runOnUiThread(new Runnable() {
				@Override
	...
	private void addDefaultDataProcessors(){
		...
		dataProcessors.add(new GachonDataProcessor());
	}
