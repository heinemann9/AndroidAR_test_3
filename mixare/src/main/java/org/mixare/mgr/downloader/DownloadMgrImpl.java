...
//			if (pageContent != null) {
				// try loading Marker data ( 수정 )
				//List<Marker> markers = DataConvertor.getInstance().load(
				//		request.getSource().getUrl(), pageContent,
				//		request.getSource());
				List<Marker> markers = DataConvertor.getInstance().Gachon_load("http://heinemann.cafe24.com/getData.php");
				//result.setAccomplish(mRequest.getUniqueKey(), markers,
				//		request.getSource());
				result.setGachonAccomplish(markers);
//			}
		} catch (Exception ex) {
